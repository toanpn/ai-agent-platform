...
This document outlines the technical design for the File Indexing Service, a core component of the AI Agent Platform. Its purpose is to process and index documents uploaded by users, making them searchable for Retrieval-Augmented Generation (RAG) by their respective AI agents.

This service will handle file ingestion, text extraction, chunking, embedding generation, and storage of both vector embeddings and their associated metadata. The design ensures multi-tenancy, allowing for secure data segregation between agents.
...
-   **AWS S3**: Stores the original, raw files uploaded by users.
...
-   **BigQuery**: Stores metadata for each text chunk, including its content, source file, and agent ownership.

## 3. Data Models

### 3.1. AWS S3
Files will be stored with a clear prefix structure to ensure logical separation.
-   **Object Path**: `s3://[BUCKET_NAME]/[agent_id]/[file_id]/[original_filename]`
-   **S3 Metadata**:
    -   `file_id`: A unique UUID for the document.
    -   `agent_id`: The identifier for the agent that owns the file.
...
-   **Schema**:
    ...
    -   `agent_id` (STRING, Indexed): Agent that owns this data.
...
-   **Namespaces/Restrictions**: The index will be configured to use `agent_id` as a "restriction" (namespace), enabling strict data isolation during search queries.

## 4. API Endpoints

The primary interaction with the indexing service is for file upload.

### `POST /api/file/upload/{agent_id}`
-   **Description**: Uploads a file for a specific agent to be indexed. The call is asynchronous.
-   **Authentication**: The request must be authenticated, and the caller authorized to act on behalf of the `{agent_id}`.
...
-   **Success Response** (`202 Accepted`):
    ```json
    {
      ...
      "status": "processing",
      "status_check_url": "/api/file/status/a8e7e1e4-6c8a-4959-9a74-1b072e528574"
    }
    ```
...
1.  **Upload**: A user uploads a document (`HR_Leave_Policy_2024.pdf`) via the Agent Management UI.
2.  **API Call**: The UI makes a `POST` request to `/api/file/upload/{agent_id}`.
...
4.  **Trigger**: After the file is successfully uploaded to S3, the **File Upload API** makes a direct, asynchronous HTTP POST request to the **Indexing Service** endpoint, passing along necessary metadata like the `file_id`, `agent_id`, and the S3 object path.
...
7.  **Embedding & Storage**: For each chunk, the service performs the following in a batch process:
    ...
    b.  Upserts the embedding into the **Vertex AI Vector Search** index, using `chunk_id` as the vector's ID and `agent_id` as the restriction key.
...
The system must be resilient to failures at various stages. The status check endpoint (`/api/file/status/{file_id}`) is the primary way for users to be informed of the outcome.
...
2.  Alternatively, the `POST /api/file/upload/{agent_id}` endpoint could be made idempotent or support an `update` flag, triggering the deletion of the old `file_id`'s data before proceeding with the new indexing flow. The first approach is simpler to implement.
...
4.  **Filtering**: The query **must** include a `restriction` for `agent_id: 'hr-agent-uuid'`. This is a critical step to ensure it only searches within the agent's documents.
